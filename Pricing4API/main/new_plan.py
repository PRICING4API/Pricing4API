...
class Plan:
    ...
    def generate_first_periodicity(self):
        """
        Genera solo la primera periodicidad (incrementos hasta la primera cuota y su meseta).
        """
        periodic_capacity = []
        accumulated_capacity = 0
        step = self.rate_wait_period.to_seconds()  # Tiempo entre solicitudes consecutivas

        # Datos relevantes
        burning_time = self.quotas_burning_times[0]  # Tiempo de quema de la primera cuota
        recovery_interval = self.quotas_recovery_intervals[0]  # Intervalo de recuperación de la primera cuota
        quota_value = self.quotes_values[0]  # Valor de la primera cuota

        # Incrementar capacidad hasta alcanzar el tiempo de quema
        while accumulated_capacity < quota_value:
            t_value = len(periodic_capacity) * step
            if t_value >= burning_time.to_seconds():  # Detener si se alcanza el tiempo de quema
                break
            accumulated_capacity += self.rate_value
            periodic_capacity.append((t_value, min(accumulated_capacity, quota_value)))

        # Añadir la meseta de recuperación
        recovery_end_time = burning_time.to_seconds() + recovery_interval.to_seconds()
        while len(periodic_capacity) * step <= recovery_end_time:
            t_value = len(periodic_capacity) * step
            capacity = accumulated_capacity if t_value < burning_time.to_seconds() else quota_value
            if t_value >= recovery_end_time:
                periodic_capacity.append((t_value, capacity+1)) 
                return periodic_capacity
            periodic_capacity.append((t_value, capacity))

    def generate_ideal_capacity_curve(self):
        """
        Genera la curva de capacidad ideal extendiendo la periodicidad base
        con mesetas hasta alcanzar el tiempo de la cuota máxima.
        """
        # Generar la primera periodicidad
        periodic_capacity = self.generate_first_periodicity()
        step = self.rate_wait_period.to_seconds()
        rate_value = self.rate_value
        quotas_values = self.quotes_values
        quotas_frequencies = self.quotes_frequencies
        max_quota = quotas_values[-1]
        max_quota_duration = quotas_frequencies[-1].to_seconds()

        # Variables auxiliares
        accumulated_capacity = periodic_capacity[-1][1]
        current_time = periodic_capacity[-1][0]
        quota_burning_time = self.quotas_burning_times[0].to_seconds()
        quota_recovery_interval = self.quotas_recovery_intervals[0].to_seconds()
        i = 2  # Empezamos en la segunda iteración
        iteration_threshold = i * (quota_burning_time + quota_recovery_interval)

        # Ajustar variables si no hay rate unitario TODO: QUE SIRVA PARA CASOS SIN RATE UNITARIO
        if not self.unitary_rate:
            rate_value = quotas_values[0]
            step = quotas_frequencies[0].to_seconds()
            quotas_values = quotas_values[1:]
            quotas_frequencies = quotas_frequencies[1:]
            quota_burning_time = self.quotas_burning_times[1].to_seconds()
            quota_recovery_interval = self.quotas_recovery_intervals[1].to_seconds()
            iteration_threshold = i * (quota_burning_time + quota_recovery_interval)

        while current_time < max_quota_duration:
            # Si alcanzamos la cuota máxima, plantamos mesetón final y salimos
            if accumulated_capacity >= max_quota:
                while current_time < max_quota_duration:
                    current_time += step
                    periodic_capacity.append((current_time, max_quota))
                break

            # Incrementar capacidad durante la iteración
            while current_time < iteration_threshold and accumulated_capacity < quotas_values[0] * i:
                current_time += step
                accumulated_capacity += rate_value
                accumulated_capacity = min(accumulated_capacity, quotas_values[0] * i)
                periodic_capacity.append((current_time, accumulated_capacity))

            # Meseta dentro de la iteración
            while current_time < iteration_threshold:
                current_time += step
                periodic_capacity.append((current_time, accumulated_capacity))

            # Subimos capacidad al alcanzar el umbral de iteración
            if current_time >= iteration_threshold:
                accumulated_capacity += rate_value
                periodic_capacity.append((current_time, accumulated_capacity))
                i += 1
                iteration_threshold = i * (quota_burning_time + quota_recovery_interval)
