...
class Plan:
    ...
    def show_available_capacity_curve(self, time_interval: TimeDuration, debug: bool = False) -> None:
        ...
        fig, (ax, rax) = plt.subplots(1, 2, figsize=(14, 6), gridspec_kw={'width_ratios': [5, 0.5]})
        rax.axis('off')

        # Añadir Radio Buttons para cambiar la unidad de tiempo
        radio_buttons = RadioButtons(rax, [tu.value for tu in TimeUnit], active=[tu.value for tu in TimeUnit].index(time_interval.unit.value))
        radio_buttons.on_clicked(lambda label: rearrange_time_axis_function(label, defined_t_values_ms, defined_capacity_values, ax, fig, self.name))
    ...
    def show_capacity_areas(self, time_interval: TimeDuration) -> None:
        ...
        # Añadir Radio Buttons para cambiar la unidad de tiempo
        radio_buttons = RadioButtons(rax, [tu.value for tu in TimeUnit], active=[tu.value for tu in TimeUnit].index(time_interval.unit.value))
        radio_buttons.on_clicked(lambda label: rearrange_time_axis_function(label, defined_t_values, defined_capacity_values, ax, fig, self.name))
